...
LightGBM is the exception to the accuracy-to-risk ordering: despite the best point MAE/RMSE it has the largest per-asset target deviation (`1.12 pp`) because it systematically under-forecasts the high-volatility assets (positions too large, realized vol about `11.5%` vs the `10%` target). A per-asset variance-RMS level calibration estimated on the validation segment (`outputs/tables/strategy_metrics.csv` row `lgb_rv_cal`, `src/strategy.py::calibrate_forecast_level`) cuts the deviation to `0.67 pp` but costs about `0.6 pp` of net annualized return, so the exception is only partially repaired at a price and LightGBM still trails GARCH (`0.38 pp`) on risk control.
