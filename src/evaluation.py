...
def test_model_comparison(frame: pd.DataFrame, *, segment: str = "test", asset: str | None = None) -> pd.DataFrame:
    ...
    metrics = walk_forward_metrics(data, forecast_columns=("historical_rv_21d", "ewma_rv", "har_rv"))
...
def regime_robustness_summary(frame: pd.DataFrame, *, models: tuple[str, ...] = ("historical_rv_21d", "ewma_rv", "har_rv")) -> pd.DataFrame:
...
def asset_robustness_summary(frame: pd.DataFrame, *, segment: str = "test", models: tuple[str, ...] = ("historical_rv_21d", "ewma_rv", "har_rv")) -> pd.DataFrame:
    ...
    metrics = walk_forward_metrics(frame.loc[frame["segment"] == segment], forecast_columns=models)
    metrics = metrics.loc[(metrics["segment"] == segment) & (metrics["asset"] != "ALL")].copy()
    for metric in ("mae", "rmse", "qlike"):
        metrics[f"{metric}_rank"] = metrics.groupby("asset")[metric].rank(method="min", ascending=True).astype("Int64")
        metrics[f"{metric}_winner"] = metrics[f"{metric}_rank"] == 1
